...
# ---------------------------------------------------------------------------
# Engine — module-level so @st.cache_resource works correctly
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Main app — wrapped in a function so `return` replaces st.stop()
# ---------------------------------------------------------------------------
def main():  # noqa: C901
    # ── ONBOARDING ────────────────────────────────────────────────────────
...
""", unsafe_allow_html=True)
        ...
        st.page_link("pages/upload.py", label="🔐 Data Analyst? Click here to upload data")
        return  # stop here — chat UI below must not render
...
""", unsafe_allow_html=True)
    ...
    # ── Suggested prompts — always visible, above chat history ────────────
    ...
    _sales_btns = [
        ...
        (
            "Top Outlet - CM v/s L3M Avg",
            f"Show top 10 outlets by current month secondary for {_scope}, "
            f"along with each outlet's last 3 month monthly average sales.",
        ),
        (
            "Unbilled Outlets",
            f"Show unbilled outlets for {_scope} — outlets that were active in the "
            f"last 3 months but have placed no orders this month MTD.",
        ),
    ]
    _prod_btns = [
        (
    ...
    with _left_col:
        st.markdown(
            '<p style="font-weight:700;font-size:0.95rem;color:#d0d5e8;'
            'margin:0.5rem 0 0.3rem;">📊 Sales</p>',
            unsafe_allow_html=True,
        )
        for _label, _query in _sales_btns:
            if st.button(_label, key=f"qs_{_label}", use_container_width=True):
                st.session_state.starter_clicked = _query
                st.rerun()
    with _right_col:
        st.markdown(
            '<p style="font-weight:700;font-size:0.95rem;color:#d0d5e8;'
            'margin:0.5rem 0 0.3rem;">⚡ Productivity</p>',
            unsafe_allow_html=True,
        )
    ...
    st.markdown("<hr style='border:1px solid #2d3250;margin:0.75rem 0;'>", unsafe_allow_html=True)
